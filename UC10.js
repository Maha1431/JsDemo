...
while (Monthdays < TotalWorkDays && TotalHrs < 160) {
  ...
  console.log(
    'UC10 showing Daily hour worked and  Earnd ' + EmpDailyHrandWageArr,
  );
...
let TotalSalary = DailyWAgesArray.filter((n) => n > 0).reduce(
  (acummulater, currentvalue) => {
...
let TotalHours = Array.from(EmpHrsMap.values()).reduce(
